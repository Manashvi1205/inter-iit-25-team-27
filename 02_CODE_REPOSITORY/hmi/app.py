...
# Config / Filenames
# -------------------------
ROVER_FILE = './data/rover_data.csv'
INVENTORY_FILE = './data/inventory.csv'   # stores inventory scan logs
RACKS_FILE = './data/racks.csv'           # stores known racks and their shelf counts

# Vision node path (for Jetson Nano)
vision_node_path = './ros_workspace/src/warehouse_scanning/scripts'
...
def update_data():
    ...
    try:
        ...
        print("Received data from rover:", data)
        if not data:
        ...
        with file_lock:
            ...
            if data.get('round_id'):
                CURRENT_ROUND_ID = int(data.get('round_id'))
            # Handle inventory scan
            if data.get('payload'):
                payload = data.get('payload', '')
                payload_list = payload.split('_')
                if len(payload_list) != 2:
                    r_part=payload_list[0]
                    s_part=payload_list[1]
                    i_part=payload_list[2]

                shelf_id=r_part+ '-' + '0' + s_part[1:]
                if payload:
                    shelf_status = i_part
                else:
...
# -------------------------
# API: Vision Control (Jetson Nano)
# -------------------------
# Global reference to vision node
_vision_node = None

def init_vision_node():
    """Initialize vision node on first request"""
    global _vision_node
    if _vision_node is None:
        try:
            ...
            print(f"Failed to initialize vision node: {e}")
            return None
    return _vision_node
...
def vision_capture():
    """
    Capture with scanning modes
    JSON params:
      - x: float (default 0.0)
      - y: float (default 0.0)
      - mode: "normal" | "top" | "bottom" | "full_sweep" (default "normal")
    """
    try:
        ...
        if vision is None:
            return jsonify({"error": "Vision node not initialized"}), 500
        
        # Map modes to capture parameters
        is_top = mode in ['top', 'full_sweep']
        is_bottom = mode in ['bottom', 'full_sweep']
        
        vision.capture(x=x, y=y, isTop=is_top, isBottom=is_bottom)
        
        return jsonify({
            "status": "success",
            "message": f"Capture initiated: mode={mode}, x={x}, y={y}",
            "mode": mode
        }), 200
...
@app.route('/api/vision/capture_once', methods=['POST'])
@login_required
def vision_capture_once():
    """
    Single frame capture at specific orientation
    JSON params:
      - x: float (default 0.0)
      - y: float (default 0.0)
      - orientation: int 0=bottom, 1=normal, 2=top (default 1)
    """
    try:
        data = request.get_json() or {}
        x = float(data.get('x', 0.0))
        ...
        vision.capture_once(x=x, y=y, orientation=orientation)
        
        return jsonify({
            "status": "success",
            "message": f"Single capture initiated at orientation {orientation}"
        }), 200
...
@app.route('/api/vision/led', methods=['POST'])
@login_required
def vision_led():
    """
    Control LED
    JSON params:
      - color: "red" | "green" | "yellow" | "white" | "blue" etc. (default "white")
    ...
    try:
        ...
        if action == 'off':
            success = vision.led_off(index=index)
            msg = f"LED {index} turned off"
        else:
            success = vision.led(color=color, level=level, index=index)
            msg = f"LED {index} set to {color} ({level})"
        
        if success:
            return jsonify({"status": "success", "message": msg}), 200
        else:
            return jsonify({"error": "LED command failed"}), 500
        
    except Exception as e:
        return jsonify({"error": str(e)}), 500


@app.route('/api/vision/servo_debug', methods=['POST'])
@login_required
def vision_servo_debug():
    """
    Open servo debug terminal (from servoCLI.py)
    """
    try:
        vision = init_vision_node()
        if vision is None:
        ...
        # Start servo debug in background
        vision.scanner.open_servo_debug_terminal()
        
        return jsonify({
            "status": "success",
            "message": "Servo debug terminal opened"
        }), 200
...
def vision_wait_completion():
    ...
    try:
        ...
        vision.wait_until_done(quiet=quiet)
